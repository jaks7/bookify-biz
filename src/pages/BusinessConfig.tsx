...
export default function BusinessConfig() {
    ...
    const fetchBusiness = async () => {
      ...
      try {
        ...
        form.reset({
          ...
          days_advance_booking: response.data.days_advance_booking || 30,
          time_advance_cancel_reschedule: response.data.time_advance_cancel_reschedule || 12,
          new_clients_can_book: response.data.new_clients_can_book !== false,
          new_clients_ask_sms_confirmation: response.data.new_clients_ask_sms_confirmation !== false,
          working_days: response.data.working_days || [1, 2, 3, 4, 5],
          public_list_business: response.data.public_list_business || false,
          public_list_services: response.data.public_list_services || false,
        });
      } catch (error) {
  ...
  const onSubmit = async (data: BusinessConfigFormValues) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.response?.data?.detail || "No se pudieron guardar los cambios",
        variant: "destructive",
      });
    }
  };
